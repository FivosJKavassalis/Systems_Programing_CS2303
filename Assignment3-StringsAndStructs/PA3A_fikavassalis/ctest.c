#include <stdio.h>
#include <string.h>
// string.h covers the C-style string functions.
#include "mystring.h"

/** ctest.c
 * Program to demonstrate character arrays and
 * dynamically-allocated memory.
 * @author Mike Ciaraldi
 * @author Fivos Kavassalis
 */

const int MAX_CHARS = 20; // Maximum number of characters in array

int main() {
	char a1[MAX_CHARS + 1]; // Character array--initially empty
	char a2[] = "Hello"; // Character array--unsized but initialized
	char a3[MAX_CHARS + 1]; // Character array--we will underfill it
	char* p1 = "Hello"; // Pointer to constant string
	char* p2 = NULL;        // Will be a pointer to dynamically-allocated string
	char* p3 = "Hi"; // Pointer to another constant string
	int copy_limit;     // Maximum characters to copy.

	strcpy(a3, "Hello, one more time."); // Initialize underfilled character array
	//mystrcpy(a3, "Hello, one more time."); // Initialize underfilled character array

	/* Print the pointers.
	 Note: this example prints
	 the values of the pointers themselves, not the targets.
	 */
	printf("Initial state:\n");
	printf("Pointers: a1 = %p, a2 = %p, a3 = %p\n", a1, a2, a3);
	printf("          p1 = %p p2 = %p\n", p1, p2);

	strcpy(a1, p3); // Initialize character array

	printf("a1 = %s\n", a1);
	printf("a2 = %s\n", a2);
	printf("a3 = %s\n", a3);

	// Note that strlen() returns size_t, which is typedef'd to
	//   unsigned long, so we need to print it with %ld instead of just %d.
	printf("For string /%s/, strlen() returns %ld\n", p1, strlen(p1));
	printf("For string /%s/, mystrlen1() returns %ld\n", p1, mystrlen1(p1));
	printf("For string /%s/, mystrlen2() returns %ld\n", p1, mystrlen2(p1));

	// Duplicate a string, using strdup(), then print
	printf("\nBefore calling strdup(), pointer a2 = %p, contents = %s\n", a2,
			a2);
	p2 = strdup(a2);
	printf("Duplicated string: \n");
	printf("Pointer p2 = %p, contents = %s\n", p2, p2);

	// Duplicate a string, using my function, then print
	printf("\nBefore calling mystrdup1(), pointer a2 = %p, contents = %s\n", a2,
			a2);
	p2 = mystrdup1(a2);
	printf("Duplicated string: \n");
	printf("Pointer p2 = %p, contents = %s\n", p2, p2);

	// Duplicate a string, using my mystrduo2(), then print
	printf("\nBefore calling mystrdup2(), pointer a2 = %p, contents = %s\n", a2,
			a2);
	p2 = mystrdup2(a2);
	printf("Duplicated string: \n");
	printf("Pointer p2 = %p, contents = %s\n", p2, p2);

	// Be sure there is a terminator character in the last element of the array
	a1[MAX_CHARS] = '\0';

	// Concatenate two character arrays, then print.
	strcpy(a1, p3); // Reset character array
	strcat(a1, a2);
	printf("Using strcat() to concatenate a2 to the end of a1\n");
	printf("a1 = %s\n", a1);

	// Concatenate two character arrays safely, then print.
	strcpy(a1, p3); // Reset character array
	a1[MAX_CHARS] = 'A'; // Remove terminator, to prove strncat() puts it back in.
	copy_limit = MAX_CHARS - strlen(a1); // How much space is left?
	printf(
			"Using strncat() to concatenate to the end of a1, with copy_limit = %d\n",
			copy_limit);
	if (copy_limit > 0)
		strncat(a1, a3, copy_limit);
	printf("a1 = %s\n", a1);

	printf("\n");

	// Testing mystrcpy()
	char str1[] = "Hellooo!";
	char str2[] = "Replacable";
	char str3[40];
	char str4[40];
	char str5[] = "WPI";

	printf("str1 is the same throughout the test case: %s\n\n", str1);

	printf("str2 before mystrcpy(): %s\n", str2);
	mystrcpy(str2, str1);
	printf("str2 after mystrcpy(): %s\n\n", str2);

	printf("str3 is not assigned a string value before mystrcpy()\n");
	mystrcpy(str3, "copy successful");
	printf("str3 after mystrcpy(): %s\n\n", str3);

	printf("str4 is not assigned a string value before mystrcpy()\n");
	mystrcpy(str4, str5);
	printf("str4 after mystrcpy(): %s\n\n", str4);

	printf("\n");

	// Testing mystrcat()
	char src[] = " Kavassalis";
	char tar[] = "Fivos";

	printf("\nSource string = %s", src);
	printf("\nTarget string = %s", tar);

	mystrcat(tar, src);

	printf("\nTarget string after strcat() is: %s\n\n", tar);

	// Other example for mystrcat() and mystrcpy()

	char source[50], dest[50];

	mystrcpy(source, "This is the source string");
	mystrcpy(dest, "This is the destination");

	mystrcat(dest, source);

	printf("Final destination string : %s\n\n", dest);

	// Testing mystrncat() (3 Examples)
	// Example for length of string greater than n
	char s[50] = "EFGHIJKL";
	char d[50] = "ABCD";

	// Appends 3 characters from s to d
	mystrncat(d, s, 3);

	// Prints the string
	printf("Source string : %s\n", s);
	printf("Destination string will concatenate 3 characters from source string : %s\n\n", d);

	// Example for length of string is exactly equal to n
	// Appends all characters from s to d
	char de[50] = "aBcD";

	mystrncat(de, s, 8);

	// Prints the string
	printf("Source string : %s\n", s);
	printf("Destination string will concatenate all characters from source string : %s\n\n", de);

	// Example for length of string is shorter than n
	char des[50] = "abcd";

	mystrncat(des, s, 15);

	// Prints the string
	printf("Source string : %s\n", s);
	printf("Destination string will concatenate all characters from source string : %s\n\n", des);

	// Testing mystrncpy()
	// Will test only for length of source string < n, so the program won't crash
	// because if length of source >= n, the function does not null-terminate the resulting string!
	char st1[] = "Hi";
	char st2[] = "Hello";

	printf("st2 before: %s\n", st2);
	mystrncpy(st2, st1, 10);
	printf("st2 after copying st1 to it: %s\n\n", st2);


	// Testing mystrndup()
	// Example for length of string > n
	char fivos[] = "Fivos Jason Kavassalis";
	printf("String to be duplicated is: %s\n", fivos);
	char* firstname = mystrndup(fivos, 5);
    printf("Duplicated string has duplicated 5 characters from initial string: %s\n\n", firstname);


	// Example for length of string = n
	char numbers[] = "OneTwoThree";
	printf("String to be duplicated is: %s\n", numbers);
    char* nums = mystrndup(numbers, 11);
    printf("Duplicated string has duplicated at most 11 characters (11 in this case): %s\n\n", nums);

    // Example for length of string < n
    char letters[] = "ABC";
    printf("String to be duplicated is: %s\n", letters);
    char* letrs = mystrndup(letters, 7);
    printf("Duplicated string has duplicated at most 7 characters (3 in this case): %s\n\n", letrs);

    return 0;
}
